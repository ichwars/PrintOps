...
A2L_LITE_GLOBAL_BASE = A2L_LITE_NORMALIZED_AMS_ID * 4
...
def a2l_lite_wire_ids(ams_id: int, tray_id: int) -> tuple[int, int, int] | None:
    """Translate a normalized A2L AMS Lite slot back to physical wire ids."""
...
def apply_tray_exist_bits(
    ...
    *,
    ...
    readers (PrintOps's AMS card, the VP slicer-facing cache, inventory short-
    ...
    1. ``_handle_ams_data`` for PrintOps's internal AMS state (printer card).
...
class PrinterState:
    ...
    # Plate dispatched by PrintOps for the current print. Some firmware versions
    ...
    # PrintOps dispatches the print itself we know the plate authoritatively;
...
class BambuMQTTClient:
    ...
    def __init__(
        ...
        # session WHEN on_print_start was suppressed (PrintOps started mid-
        ...
        # Intercepts slicer/PrintOps print commands to get the slot-to-tray mapping
        self._captured_ams_mapping: list[int] | None = None

        # A2L reports its AMS Lite as physical unit id 16, while its tray bits
        # use the id-6 base. Normalize at ingest and remember that tray_now is
        # local 0-3 for this unit.
        ...
        if self.state.connected and self.is_stale():
            ...
            if self._report_messages_since_connect == 0 and not self._zero_report_hint_logged:
                ...
                logger.warning(
                    ...
                    "case-sensitive. Verify the serial number configured in PrintOps "
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            # Intercept request-topic messages (print commands from slicer/PrintOps)
    ...
    def _resolve_local_slot_from_mapping(local_slot: int, mapping_raw: list | None) -> int | None:
        ...
        for value in mapping_raw:
            ...
            elif ams_hw_id == A2L_LITE_PHYSICAL_AMS_ID and (slot & 0x03) == local_slot:
                candidates.add(A2L_LITE_GLOBAL_BASE + local_slot)
    ...
    def _normalize_a2l_am_units(self, ams_list) -> None:
        """Rewrite A2L AMS Lite unit id 16 to normalized id 6 in-place."""
        ...
        for unit in ams_list:
            ...
            if uid == A2L_LITE_PHYSICAL_AMS_ID:
                ...
                if not self._has_a2l_am_unit:
                    logger.info(
                        "[%s] A2L AMS Lite detected (unit id 16); normalizing to id %d",
                        self.serial_number,
    ...
    def _handle_ams_data(self, ams_data):
        ...
        if isinstance(ams_data, dict):
            ...
            if "tray_now" in ams_data:
                ...
                if self._is_dual_nozzle and 0 <= parsed_tray_now <= 3:
                    ...
                    # or before PrintOps connected) fall through unchanged.
                    ...
                    if captured and all(s == -1 for s in captured):
                        ...
                        if self._has_a2l_am_unit and num_ams <= 1:
                            # A2L AMS Lite reports tray_now as a local slot.
                            ...
                            if self._has_a2l_am_unit:
                                logger.warning(
                                    "[%s] A2L AMS Lite alongside another AMS unit is not fully resolved by "
                                    "tray_now alone; using mapping fallback",
                                    self.serial_number,
                                )
        ...
        # sync with PrintOps's AMS card (#1726). See the helper's docstring
    ...
    def _update_state(self, data: dict):
        ...
        is_new_print = (
            ...
            and self._previous_gcode_state is not None  # #1304: skip on first push after PrintOps startup
        ...
        if self.state.state == "RUNNING" and current_file:
            if not self._was_running:
                ...
                # (PrintOps started mid-print — the #1304 guard suppressed
        ...
        if (is_new_print or is_file_change) and self.on_print_start:
            ...
            # Restart-recovery hook (#1485 follow-up): PrintOps started mid-
    ...
    def connect(self, loop: asyncio.AbstractEventLoop | None = None):
        ...
        client_id = f"printops_{self.serial_number}_{os.getpid()}_{BambuMQTTClient._client_instance_counter}"
    ...
    def start_print(
        ...
        if self._client and self.state.connected:
            ...
            if ams_mapping is not None:
                for tray_id in ams_mapping:
                    ...
                    if tray_id == -1:
                        ...
                        # A2L AMS Lite: normalized global 24-27 becomes local
                        # flat slot 0-3 plus physical unit id 16 in ams_mapping2.
                        wire_ams, wire_slot, _ = _a2l
                        flat_ams_mapping.append(wire_slot)
                        ams_mapping2.append({"ams_id": wire_ams, "slot_id": wire_slot})
                    else:
            ...
            # On single-nozzle printers, a resolved real tray is authoritative:
            # it forces use_ams=True even if a Virtual Printer row carried a stale
            # false. Explicit external trays force it false. Unresolved -1 does
            # neither and must not be treated as external (#2589/#2595).
            if ams_mapping and not is_dual_nozzle:
                ...
                if has_real_tray and not use_ams:
                    use_ams = True
                    logger.info(
                        "[%s] AMS mapping resolved a real slot; setting use_ams=True",
                        self.serial_number,
                    )
    ...
    def execute_hms_action(self, print_error: str, action: str, job_id: str | None = None) -> bool:
        ...
        def hms_ignore_command():
            ...
            # paused print in a single operation. The previous PrintOps code
