"""Restore supported PrintOps data from a Git provider backup (issues #2656/#118).
...
_PROTECTED_SETTING_PREFIXES = (
    "ldap_",
    # PrintOps commercial configuration is relational and versioned through
    # the order/document/calculation APIs. Restoring legacy Settings rows would
    # bypass those invariants and can reintroduce foreign primary keys.
    "orders.",
    "documents.",
    "calculations.",
)
...
class GitHubRestoreService:
    ...
    async def _count_items(
        ...
        if category == RestoreCategory.ARCHIVES:
            ...
            return count, _Detail(
                "archivesMetadataAndRelationsDetached",
                "Metadata only — 3MF files and thumbnails are not in a Git backup; numeric printer/project "
                "links are not portable and PrintOps order, document, and calculation links are not changed",
            )
    ...
    async def _restore_archives(
        ...
        for entry in archives:
            ...
            # rows at the same numbers that represent different printers or
            # projects. Never copy those ids from the backup. New history rows
            # remain detached; an overwrite keeps the locally resolved links.
            # Order/document/calculation relations live in separate PrintOps
            # tables and are deliberately neither inferred nor modified here.
            if entry.get("printer_id") is not None or entry.get("project_id") is not None:
                tally.note(
                    "archivesRelationsDetached",
                    "Archive printer/project links were not restored because backup primary keys are not portable; "
                    "existing local links were kept and new history rows remain detached. Order, document, and "
                    "calculation links were not changed.",
                )
    ...
    async def _restore_spool_usage(
        ...
        for entry in usage:
            ...
            # As with archive links, a numeric printer id from another instance
            # has no portable meaning. Usage remains useful without it.
            printer_id = None
            if entry.get("printer_id") is not None:
                unlinked_printers += 1
        ...
        if unlinked_printers:
            tally.note(
                "spoolUsagePrinterUnlinked",
                f"{unlinked_printers} usage record(s) were restored without a printer link because backup "
                "primary keys are not portable.",
                count=unlinked_printers,
            )
