...
POLL_INTERVAL = 15
STALE_AFTER = timedelta(seconds=POLL_INTERVAL * 3)
CRITICAL_DEVICE_CLASSES = {"smoke", "gas", "moisture", "problem", "safety"}
...
class HASensorManager:
    ...
    def _fails_closed(sensor: PrinterHASensor) -> bool:
        strategy = getattr(sensor, "failure_strategy", "auto")
        if strategy == "fail_closed":
            return True
        if strategy == "fail_open":
            return False
        return getattr(sensor, "device_class", None) in CRITICAL_DEVICE_CLASSES
    ...
    async def blocked_printers(self, db: AsyncSession) -> dict[int, str]:
        ...
        for sensor in result.scalars().all():
            ...
            if reading and reading.reachable and not stale and reading.alerting:
                blocked.setdefault(sensor.printer_id, []).append(sensor.name)
            elif (reading is None or not reading.reachable or stale) and self._fails_closed(sensor):
                # A manual override applies only to uncertainty. A positively
                # observed unsafe state remains an interlock.
                if sensor.printer_id not in self._overrides:
                    blocked.setdefault(sensor.printer_id, []).append(f"{sensor.name} (unavailable)")
