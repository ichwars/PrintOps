...
Auth shape: OAuth 2.0 Device Authorization Grant (RFC 8628). PrintOps is a
...
user approves a short ``user_code`` in their Orca Cloud settings; PrintOps
...
``PrintOps/<version>`` UA clears it. No TLS-fingerprint matching needed.
"""
...
# Scope requested at pairing time. PrintOps currently only READS the user's
...
# PrintOps without impersonating Orca's desktop client. Also the thing that
...
class OrcaCloudService:
    ...
    async def introspect(self) -> dict[str, Any]:
        ...
        after pairing to record the user's id for display in PrintOps's UI."""
