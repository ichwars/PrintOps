...
class User(Base):
    ...
    # Per-user Orca Cloud credentials. Orca uses external-app device pairing
    # with expiring access tokens and rotating single-use refresh tokens, so we
    # store the refresh token + expiry alongside the access token.
    ...
    # Transient device-code state held between /orca-cloud/device/start and
    # /orca-cloud/device/poll. Cleared on successful pairing; expires after 10
    # minutes if the user abandons the flow.
