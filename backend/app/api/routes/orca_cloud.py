...
Auth shape (see :mod:`backend.app.services.orca_cloud` for the deep dive):
    ...
    POST /orca-cloud/logout
        Clear stored tokens. The user can also disconnect from Orca Cloud's
        own settings to revoke server-side.
...
async def device_start(
    ...
    try:
        data = await svc.request_device_code(instance_url=instance_url, instance_label="PrintOps")
    except OrcaCloudAuthError as e:
...
async def logout(
    ...
    PrintOps will no longer have the token to use either way)."""
