...
async def _get_group_or_404(
    ...
    """Load a group without exposing another user's group through its ID."""
    query = select(FileVariantGroup).where(FileVariantGroup.id == group_id)
    if user is not None and not can_access_all:
        query = query.where(FileVariantGroup.created_by_id == user.id)
...
async def get_group_for_file(
    ...
    return await _group_response(
        db,
        await _get_group_or_404(db, lib_file.variant_group_id, user, can_read_all),
    )
...
async def get_variant_group(
    ...
    user, can_read_all = auth_result
    return await _group_response(db, await _get_group_or_404(db, group_id, user, can_read_all))
...
async def update_variant_group(
    ...
    """
    user, can_update_all = auth_result
    group = await _get_group_or_404(db, group_id, user, can_update_all)
