...
class AppSettings(BaseModel):
    ...
    show_developer_lan_warning: bool = Field(
        default=True, description="Show warning banner for printers without Developer LAN mode"
    )
    ...
    queue_max_concurrent_uploads: int = Field(
        ...
        le=16,
        description=(
            "How many printers the queue may upload to at the same time. Each concurrent upload "
            "uses one printer connection and one worker thread."
        ),
    )
