...
async def test_availability_preview_checks_both_stores_without_reserving(async_client, db_session):
    ...
    db_session.add(SmallPartLedgerEntry(small_part_id=part.id, entry_kind="opening", physical_delta=5, reserved_delta=0, reason="Opening", idempotency_key="availability-opening"))
    await db_session.commit()

    response = await async_client.post("/api/v1/calculations/availability-preview", json={
        "name": "Standard",
        "is_preferred": True,
        "plates": [{"project_plate_id": 1, "good_parts": 4, "parts_per_print": 2, "material_code": "PETG", "grams_per_print": "100"}],
        "small_parts": [{"small_part_id": part.id, "quantity": "6", "description_snapshot": "Screw", "unit_code_snapshot": "C62", "unit_cost_snapshot": "0.05"}],
    })
    ...
    assert await db_session.scalar(select(func.count()).select_from(SmallPartLedgerEntry).where(SmallPartLedgerEntry.entry_kind == "reservation")) == 0
