...
class TestVirtualPrinterInstance:
    ...
    async def test_add_to_print_queue_force_color_match_carries_tray_info_idx(self, tmp_path):
        """#2650: force-color overrides keep the 3MF's Bambu variant id so
        PLA Basic/Matte/Silk spools with the same colour can be distinguished.
        """
        from backend.app.services.virtual_printer.manager import VirtualPrinterInstance
...
class TestBindServer:
    ...
    def test_build_frame_roundtrip(self, bind_server):
        ...
        assert parsed["login"]["name"] == "PrintOps"
    ...
    def test_bind_server_stores_config(self, bind_server):
        ...
        assert bind_server.name == "PrintOps"
