...
class TestRestoreSettings:
    ...
    @pytest.mark.asyncio
    async def test_printops_commercial_settings_are_never_restored(self, db_session):
        tally = _CategoryTally()
        payload = {
            "settings": {
                "currency": "EUR",
                "orders.payment_term_days": "7",
                "documents.default_layout_id": "42",
                "calculation_defaults": '{"defaultPrinterId":99}',
            }
        }
        ...
        assert rows == {"currency": "EUR"}
        assert tally.restored == 1
        assert _is_protected_setting_key("orders.payment_term_days")
        assert _is_protected_setting_key("documents.default_layout_id")
        assert _is_protected_setting_key("calculation_defaults")
...
class TestRestoreSpools:
    ...
    async def test_dangling_printer_id_is_cleared(self, db_session):
        ...
        assert "spoolUsagePrinterUnlinked" in _codes(tally)
...
class TestRestoreArchives:
    ...
    async def test_dangling_printer_and_project_links_are_cleared(self, db_session):
        ...
        assert "archivesRelationsDetached" in _codes(tally)

    @pytest.mark.asyncio
    async def test_even_a_current_numeric_printer_id_is_not_reused(self, db_session, printer_factory):
    ...
    @pytest.mark.asyncio
    async def test_overwrite_preserves_locally_resolved_links(self, db_session, printer_factory):
        printer = await printer_factory()
        ...
        entry = self._archive_entry(printer_id=9999, notes="restored")
        await _service()._restore_archives(db_session, {"archives": [entry]}, True, tally, {})
        await db_session.commit()

        await db_session.refresh(archive)
        assert archive.printer_id == printer.id
        assert archive.notes == "restored"
