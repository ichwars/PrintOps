...
class TestBlockedPrinters:
    ...
    @pytest.mark.asyncio
    async def test_critical_sensor_fails_closed_when_unreachable(self):
        sensor = _sensor(block_print=True, device_class="smoke")
        manager, db = self._manager_with([sensor], {1: SensorReading(None, None, False, False)})

        assert await manager.blocked_printers(db) == {4: "Enclosure Door (unavailable)"}

    @pytest.mark.asyncio
    async def test_explicit_fail_open_overrides_critical_default(self):
        sensor = _sensor(block_print=True, device_class="smoke", failure_strategy="fail_open")
        manager, db = self._manager_with([sensor], {1: SensorReading(None, None, False, False)})

        assert await manager.blocked_printers(db) == {}

    @pytest.mark.asyncio
    async def test_manual_override_bypasses_fail_closed_uncertainty(self):
        sensor = _sensor(block_print=True, device_class="safety")
        manager, db = self._manager_with([sensor], {})
        manager.set_interlock_override(4, username="operator", reason="Verified enclosure locally")
