"""Tests for Pushover emergency-priority retry/expire handling (#2586)."""
...
class _CaptureClient:
    """Minimal stand-in for httpx.AsyncClient that records posted data."""
...
async def test_priority_2_clamps_to_pushover_limits(service_with_capture):
    ...
    assert ok
    assert client.last_data["retry"] == 30
    assert client.last_data["expire"] == 10800
