...
_Changelog truncated — see the full [CHANGELOG.md](https://github.com/ichwars/PrintOps/blob/main/CHANGELOG.md) for the complete list._"
    fi

    # Build pull commands for the release body
    PULL_COMMANDS=""
    if [ "$PUSH_GHCR" = true ]; then
        PULL_COMMANDS="docker pull ghcr.io/ichwars/printops:daily"
    fi
    if [ "$PUSH_DOCKERHUB" = true ]; then
        if [ -n "$PULL_COMMANDS" ]; then
            PULL_COMMANDS="${PULL_COMMANDS}
# or
docker pull ichwars/PrintOps:daily"
        else
            PULL_COMMANDS="docker pull ichwars/PrintOps:daily"
        fi
    fi
