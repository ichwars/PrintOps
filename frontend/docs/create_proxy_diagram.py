#!/usr/bin/env python3
"""
Create a professional network architecture diagram for PrintOps Virtual Printer Proxy Mode.
...
def create_diagram():
    ...
    # Subtitle
    subtitle = "Secure remote printing through PrintOps"
    ...
    # PrintOps section (center-right)
    printops_x = 700
    printops_box = [560, 140, 840, 500]
    ...
    draw.text((remote_x, section_y + 100), "Connects to PrintOps", font=fonts['small'], fill=TEXT_LABEL, anchor="mm")
    ...
    # === PRINTOPS SERVER ===
    draw_rounded_rect(draw, printops_box, 12, fill=CONTAINER_BG, outline=BAMBU_GREEN_DIM, width=2)
    draw.text((printops_x, 165), "PRINTOPS SERVER", font=fonts['label'], fill=BAMBU_GREEN, anchor="mm")

    # Server icon
    draw_server_icon(draw, printops_x, section_y - 50, 70, BAMBU_GREEN)
    draw.text((printops_x, section_y + 20), "TLS Proxy", font=fonts['heading'], fill=TEXT_PRIMARY, anchor="mm")

    # Incoming ports (left side of PrintOps)
    draw.text((printops_x, section_y + 70), "LISTEN PORTS", font=fonts['small'], fill=TEXT_LABEL, anchor="mm")
    draw_rounded_rect(draw, [printops_x - 55, section_y + 85, printops_x + 55, section_y + 130],
                      6, fill=(35, 35, 45), outline=CONTAINER_BORDER, width=1)
    draw.text((printops_x, section_y + 98), "FTP", font=fonts['small'], fill=TEXT_SECONDARY, anchor="mm")
    draw.text((printops_x, section_y + 115), "990", font=fonts['port'], fill=BAMBU_GREEN, anchor="mm")

    draw_rounded_rect(draw, [printops_x - 55, section_y + 140, printops_x + 55, section_y + 185],
                      6, fill=(35, 35, 45), outline=CONTAINER_BORDER, width=1)
    draw.text((printops_x, section_y + 153), "MQTT", font=fonts['small'], fill=TEXT_SECONDARY, anchor="mm")
    draw.text((printops_x, section_y + 170), "8883", font=fonts['port'], fill=BAMBU_GREEN, anchor="mm")
    ...
    # TLS badge between PrintOps and printer
    ...
    # Flow description
    draw.text((WIDTH // 2, info_y), "← Slicer traffic encrypted and relayed through PrintOps to your printer →",
    ...
    # PrintOps branding
    draw.text((WIDTH // 2, HEIGHT - 30), "github.com/ichwars/PrintOps", font=fonts['small'], fill=TEXT_LABEL, anchor="mm")
...
def main():
    ...
    output_path = Path("/opt/claude/projects/printops/docs/images/proxy-mode-diagram.png")
    ...
    frontend_path = Path("/opt/claude/projects/printops/frontend/docs/proxy-mode-diagram.png")
