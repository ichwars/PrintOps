  ...
  return (
    <div className="w-full space-y-5 p-4 md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2"><LegacySelect value={templateId} onChange={event => setTemplateId(event.target.value)} aria-label={de ? 'Vorlage auswählen' : 'Select template'} className="h-10 rounded-lg border border-bambu-dark-tertiary bg-bambu-dark px-3 text-white"><option value="">{de ? 'Vorlage auswählen…' : 'Select template…'}</option>{templates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}</LegacySelect><button type="button" onClick={() => void instantiate()} disabled={!templateId} className="inline-flex h-10 items-center gap-2 rounded-lg bg-bambu-dark px-4 text-white disabled:opacity-50"><FileText className="h-4 w-4" />{de ? 'Aus Vorlage' : 'From template'}</button><button type="button" onClick={() => setEditor('create')} className="inline-flex h-10 items-center gap-2 rounded-lg bg-bambu-green px-4 font-medium text-black"><Plus className="h-4 w-4" />{de ? 'Kalkulation hinzufügen' : 'Add calculation'}</button></div>
      </div>
