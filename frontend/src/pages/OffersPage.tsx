...
export function OffersPage() {
  ...
  return <div className="space-y-5 p-4 md:p-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="flex items-center gap-3 text-2xl font-bold text-white"><FileText className="h-7 w-7 text-bambu-green" />{de ? 'Angebote' : 'Offers'}</h1><p className="mt-1 text-bambu-gray">{de ? 'Entwurf, Versand und Annahme mit verbindlicher Lagerreservierung.' : 'Draft, send, and accept with binding stock reservation.'}</p></div><Select ariaLabel={de ? 'Angebotsstatus filtern' : 'Filter offer status'} value={filter} onValueChange={(value) => setFilter(value)} options={[{ value: '', label: de ? 'Alle Status' : 'All statuses' }, { value: 'draft', label: de ? 'Entwurf' : 'Draft' }, { value: 'sent', label: de ? 'Versendet' : 'Sent' }, { value: 'accepted', label: de ? 'Angenommen' : 'Accepted' }, { value: 'rejected', label: de ? 'Abgelehnt' : 'Rejected' }]} controlClassName="h-10 min-w-44" /></div>
