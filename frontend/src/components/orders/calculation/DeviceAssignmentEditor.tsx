...
export function DeviceAssignmentEditor({ operation, printers, dryers, defaultPrinterId, defaultDryerId, locale, onChange }: { operation: CalculationOperation; printers: Printer[]; dryers: Equipment[]; defaultPrinterId: number; defaultDryerId: number; locale: string; onChange: (operation: CalculationOperation) => void }) {
  ...
  return <div className="grid gap-3 rounded-lg border border-bambu-dark-tertiary bg-bambu-dark-secondary/50 p-3 sm:grid-cols-3 lg:col-span-7"><label className="text-xs text-bambu-gray"><span className="flex items-center gap-1"><Cpu className="h-3.5 w-3.5" />{de ? 'Drucker' : 'Printer'}</span><select value={printerId} onChange={e => assignPrinter(Number(e.target.value))} className={inputClass}><option value={0}>{de ? 'Kein Drucker' : 'No printer'}</option>{printers.filter(item => item.is_active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-xs text-bambu-gray"><span className="flex items-center gap-1"><Wind className="h-3.5 w-3.5" />{de ? 'Trockner' : 'Dryer'}</span><select value={dryerId} onChange={e => assignDryer(Number(e.target.value))} className={inputClass}><option value={0}>{de ? 'Kein Trockner' : 'No dryer'}</option>{dryers.filter(item => item.is_active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-xs text-bambu-gray">{de ? 'Trocknungsdauer h' : 'Drying hours'}<input type="number" min="0" step="0.05" value={dryingHours} onChange={e => onChange({ ...operation, provenance: { ...operation.provenance, drying_hours: Number(e.target.value) } })} className={inputClass} /></label></div>;
}
