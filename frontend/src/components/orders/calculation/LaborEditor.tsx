...
export function LaborEditor({ operation, locale, onChange }: { operation: CalculationOperation; locale: string; onChange: (operation: CalculationOperation) => void }) {
  ...
  return <div className="mt-3 border-t border-bambu-dark-tertiary pt-3 lg:col-span-7">
    ...
    {operation.labor.map((entry, index) => <div key={index} className="mt-2 grid gap-2 sm:grid-cols-[1fr_110px_130px_150px_40px]">
      <input aria-label={de ? 'Tätigkeit' : 'Activity'} value={entry.kind} onChange={e => update(index, 'kind', e.target.value)} className={inputClass} />
      <input aria-label={de ? 'Stunden' : 'Hours'} type="number" min="0" step="0.05" value={entry.hours} onChange={e => update(index, 'hours', e.target.value)} className={inputClass} />
      <input aria-label={de ? 'Stundensatz' : 'Hourly rate'} type="number" min="0" step="0.01" value={entry.hourly_rate} onChange={e => update(index, 'hourly_rate', e.target.value)} className={inputClass} />
      <select aria-label={de ? 'Zuordnung' : 'Allocation'} value={entry.allocation_basis} onChange={e => update(index, 'allocation_basis', e.target.value)} className={inputClass}><option value="request">{de ? 'je Anfrage' : 'per request'}</option><option value="run">{de ? 'je Lauf' : 'per run'}</option><option value="unit">{de ? 'je Stück' : 'per unit'}</option></select>
