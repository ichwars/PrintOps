...
import { api, ApiError, type AppSettings, type BusinessProfile, type BusinessProfileOption, type CustomerListItem, type Equipment, type InventorySpool, type Printer, type ProjectCreate, type ProjectListItem } from '../../api/client';
...
export function CalculationWorkspace({ calculation, onClose, onSaved, locale }: Props) {
  ...
  const [persistedCalculation, setPersistedCalculation] = useState<CalculationDetail | null>(calculation);
  ...
  useEffect(() => { setPersistedCalculation(calculation); }, [calculation]);
  useEffect(() => { void api.getBusinessProfiles(false).then(items => { const active = items.filter(item => item.is_active); setBusinessProfiles(items); setProfiles(active); if (!persistedCalculation && draft.business_profile_id === 0 && active.length) setDraft(current => ({ ...current, business_profile_id: (active.find(item => item.is_default) ?? active[0]).id, currency: (active.find(item => item.is_default) ?? active[0]).default_currency || current.currency })); }); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => { if (persistedCalculation) void calculationsApi.revisions(persistedCalculation.id).then(setRevisions); }, [persistedCalculation]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const selectedProfile = businessProfiles.find(item => item.id === draft.business_profile_id);
    const profileTaxRate = selectedProfile ? (selectedProfile.tax_mode === 'standard' ? Number(selectedProfile.default_tax_rate ?? 0) / 100 : 0) : n('taxPercent', 19) / 100;
    const laborRate = String(ov.labor_rate ?? n('laborRate', 20));
    const defaultLabor = [
      { kind: 'setup', hours: String(ov.setup_hours ?? n('setupHours', 0.3)), hourly_rate: laborRate, allocation_basis: 'request' as const, sort_order: 0 },
      { kind: 'post_processing', hours: String(ov.post_processing_hours_per_unit ?? n('postProcessingHours', 0.25)), hourly_rate: laborRate, allocation_basis: 'unit' as const, sort_order: 1 },
      { kind: 'cad', hours: String(ov.cad_hours ?? n('cadHours', 0)), hourly_rate: laborRate, allocation_basis: 'request' as const, sort_order: 2 },
      { kind: 'qa', hours: String(ov.qa_hours ?? n('qaHours', 0.05)), hourly_rate: laborRate, allocation_basis: 'request' as const, sort_order: 3 },
    ];
    ...
    const operationInputs = productionOperations.map(operation => {
      ...
      const dryingHours = Number(operation.provenance.drying_hours ?? n('dryingHours'));
      return { good_parts: operation.good_parts, parts_per_run: operation.parts_per_run, scrap_runs: operation.scrap_runs, material_grams_per_run: operation.material_grams_per_run, material_price_per_kg: String(ov.material_price_per_kg ?? settings.default_filament_cost), material_markup_rate: String(ov.material_markup_rate ?? n('materialMarkupPercent') / 100), print_hours_per_run: operation.print_hours_per_run, machine_cost_per_hour: printer?.hourly_rate ?? '0', printer_power_kw: String(Number(printer?.nominal_power_watts ?? 0) / 1000), electricity_price_per_kwh: String(settings.energy_cost_per_kwh), drying_hours: String(dryingHours), dryer_power_kw: String(Number(dryer?.nominal_power_watts ?? 0) / 1000), labor: operation.labor.length ? operation.labor : defaultLabor, consumables: '0', packaging: '0', additional_costs: String(Number(dryer?.hourly_rate ?? 0) * dryingHours), additive_materials: '0', scrap_rate: '0', risk_rate: '0', shipping: '0', price_method: 'markup' as const, price_rate: '0', explicit_price: '0', discount_rate: '0', tax_rate: '0', minimum_price: '0', minimum_profit: '0', rounding_mode: 'none' as const };
    });
  ...
  }, [businessProfiles, draft.business_profile_id, draft.commercial_overrides, draft.quantity, dryers, printers, selected, settings]);
  const ensureCalculationForUpload = async (file: File) => {
    if (persistedCalculation) return persistedCalculation.id;
    const fallbackTitle = file.name.replace(/\.3mf$/i, '').trim() || (de ? '3MF-Projekt' : '3MF project');
    const draftToPersist = draft.title.trim() ? draft : { ...draft, title: fallbackTitle };
    setSaving(true); setMessage(null); setConflict(false);
    try {
      const created = await calculationsApi.create(draftToPersist);
      setDraft(draftToPersist);
      setPersistedCalculation(created);
      setAutoSavedDraft({ id: created.id, version: created.version });
      setMessage(de ? 'Entwurf automatisch gespeichert.' : 'Draft saved automatically.');
      return created.id;
    } catch (error) {
  ...
  const save = async () => { setSaving(true); setMessage(null); setConflict(false); try { if (persistedCalculation) await calculationsApi.update(persistedCalculation.id, { ...draft, expected_version: persistedCalculation.version }); else await calculationsApi.create(draft); setAutoSavedDraft(null); onSaved(); } catch (error) { const stale = error instanceof ApiError && error.status === 409; setConflict(stale); setMessage(stale ? (de ? 'Die Serverversion ist neuer. Deine Eingaben bleiben erhalten.' : 'The server version is newer. Your input is preserved.') : error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  const reloadServerVersion = async () => { if (!persistedCalculation) return; const current = await calculationsApi.get(persistedCalculation.id); setPersistedCalculation(current); setDraft({ business_profile_id: current.business_profile_id, customer_id: current.customer_id, project_id: current.project_id, request_kind: current.request_kind, quantity: current.quantity, title: current.title, position_description: current.position_description, special_terms: current.special_terms, commercial_overrides: { ...current.commercial_overrides }, currency: current.currency, notes: current.notes, variants: structuredClone(current.variants) }); setConflict(false); setMessage(null); };
  const createProjectFromRequest = async (data: ProjectCreate) => {
    const created = await api.createProject(data);
    const updatedProjects = await api.getProjects('active');
    setProjects(updatedProjects);
    return created.id;
  };
  const approve = async () => { if (!persistedCalculation) return; setSaving(true); setMessage(null); try { const validation = await calculationsApi.validate(persistedCalculation.id); if (validation.blockers.length) { setMessage(`${de ? 'Freigabe blockiert' : 'Approval blocked'}: ${validation.blockers.join(', ')}`); return; } if (validation.warnings.length && approvalWarnings.length === 0) { setApprovalWarnings(validation.warnings); return; } await calculationsApi.approve(persistedCalculation.id, persistedCalculation.version, warningReasons); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  const revise = async () => { if (!persistedCalculation) return; setSaving(true); try { const created = await calculationsApi.revise(persistedCalculation.id); setMessage(`${de ? 'Folgeversion angelegt' : 'Follow-up version created'}: K-${String(created.id).padStart(6, '0')}`); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  const archive = async () => { if (!persistedCalculation || !window.confirm(de ? 'Kalkulation wirklich archivieren?' : 'Archive this calculation?')) return; setSaving(true); try { await calculationsApi.archive(persistedCalculation.id, persistedCalculation.version); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  const template = async () => { if (!persistedCalculation || !templateName.trim()) return; await calculationsApi.createTemplate(persistedCalculation.id, templateName.trim()); setTemplateName(''); setMessage(de ? 'Vorlage gespeichert.' : 'Template saved.'); };
  const closeWorkspace = async () => { if (!autoSavedDraft) { onClose(); return; } setSaving(true); setMessage(null); try { await calculationsApi.remove(autoSavedDraft.id, autoSavedDraft.version); setAutoSavedDraft(null); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  const deleteExistingDraft = async () => { if (!calculation || !persistedCalculation || persistedCalculation.status !== 'draft') return; setSaving(true); setMessage(null); try { await calculationsApi.remove(persistedCalculation.id, persistedCalculation.version); setDeleteConfirmOpen(false); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error'); } finally { setSaving(false); } };
  return <><div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/70 p-3 md:p-8"><div role="dialog" aria-modal="true" aria-labelledby={titleId} className="mx-auto flex max-h-full w-full max-w-7xl flex-col overflow-hidden rounded-xl border border-bambu-dark-tertiary bg-bambu-dark-secondary shadow-2xl">
    <header className="flex shrink-0 items-center justify-between border-b border-bambu-dark-tertiary bg-bambu-dark-secondary px-5 py-4"><div><h2 id={titleId} className="text-xl font-bold text-white">{persistedCalculation ? persistedCalculation.title : de ? 'Kalkulation hinzufügen' : 'Add calculation'}</h2><p className="text-sm text-bambu-gray">{persistedCalculation ? `K-${String(persistedCalculation.id).padStart(6, '0')} · v${persistedCalculation.version}` : de ? 'Konkrete Kundenanfrage' : 'Concrete customer request'}</p></div><button type="button" onClick={() => void closeWorkspace()} disabled={saving} aria-label={de ? 'Schließen' : 'Close'} className="rounded p-2 text-bambu-gray hover:bg-bambu-dark disabled:opacity-50"><X /></button></header>
    ...
    <main className="space-y-5 p-5">
      ...
      <RequestEditor draft={draft} profiles={profiles} customers={customers} projects={projects} locale={locale} onChange={setDraft} onCreateProject={createProjectFromRequest} />
      ...
      <ProjectFileSection calculationId={persistedCalculation?.id ?? null} plates={selected.plates} printers={printers} dryers={dryers} spools={spools} locale={locale} onEnsureCalculation={ensureCalculationForUpload} onChange={(plates) => changeVariant(selectedIndex, (variant) => ({ ...variant, plates }))} />
      ...
      <AvailabilityPanel calculationId={persistedCalculation && persistedCalculation.status !== 'draft' ? persistedCalculation.id : null} variant={selected} locale={locale} />
      <CommercialOverridesEditor values={draft.commercial_overrides} defaults={effectiveDefaults} locale={locale} onChange={commercial_overrides => setDraft(current => ({ ...current, commercial_overrides }))} />
      ...
      <FollowUpActions locale={locale} calculationRevisionId={persistedCalculation && persistedCalculation.status === 'approved' ? revisions[0]?.id ?? null : null} />
    </main>
    </ScrollArea>
    <footer className="flex shrink-0 flex-wrap justify-end gap-3 border-t border-bambu-dark-tertiary bg-bambu-dark-secondary px-5 py-4">{calculation && persistedCalculation?.status === 'draft' && <button type="button" onClick={() => setDeleteConfirmOpen(true)} disabled={saving} className="mr-auto inline-flex h-10 items-center gap-2 rounded-lg border border-red-500/60 px-4 text-red-300 hover:bg-red-500/10 disabled:opacity-50"><Trash2 className="h-4 w-4" />{de ? 'Löschen' : 'Delete'}</button>}{persistedCalculation && ['approved', 'superseded'].includes(persistedCalculation.status) && <button type="button" onClick={() => void archive()} disabled={saving} className="mr-auto inline-flex h-10 items-center gap-2 rounded-lg border border-bambu-dark-tertiary px-4 text-bambu-gray hover:text-white disabled:opacity-50"><Archive className="h-4 w-4" />{de ? 'Archivieren' : 'Archive'}</button>}{persistedCalculation && ['approved', 'superseded'].includes(persistedCalculation.status) && <button onClick={() => void revise()} disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-lg border border-bambu-green px-4 text-bambu-green disabled:opacity-50"><GitBranch className="h-4 w-4" />{de ? 'Neue Version' : 'New version'}</button>}<button type="button" onClick={() => void closeWorkspace()} disabled={saving} className="h-10 rounded-lg bg-bambu-dark px-4 text-white disabled:opacity-50">{de ? 'Abbrechen' : 'Cancel'}</button>{persistedCalculation?.status === 'draft' && <button onClick={() => void approve()} disabled={saving || blockers.length > 0 || (approvalWarnings.length > 0 && approvalWarnings.some(code => !warningReasons[code]?.trim()))} className="inline-flex h-10 items-center gap-2 rounded-lg border border-bambu-green px-4 text-bambu-green disabled:opacity-50"><Check className="h-4 w-4" />{de ? 'Freigeben' : 'Approve'}</button>}{(!persistedCalculation || persistedCalculation.status === 'draft') && <button onClick={() => void save()} disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-lg bg-bambu-green px-4 font-medium text-black disabled:opacity-50"><Save className="h-4 w-4" />{de ? 'Speichern' : 'Save'}</button>}</footer>
  </div></div>{deleteConfirmOpen && persistedCalculation && <ConfirmModal title={de ? 'Kalkulation löschen?' : 'Delete calculation?'} message={de ? 'Die Kalkulation und alle hochgeladenen Projektdateien werden endgültig gelöscht.' : 'The calculation and all uploaded project files will be permanently removed.'} confirmText={de ? 'Löschen' : 'Delete'} variant="danger" isLoading={saving} onCancel={() => setDeleteConfirmOpen(false)} onConfirm={() => void deleteExistingDraft()} />}</>;
}
