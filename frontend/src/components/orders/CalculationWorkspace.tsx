...
export function CalculationWorkspace({ calculation, onClose, onSaved, locale }: Props) {
  ...
  useEffect(() => { void api.getBusinessProfileOptions().then(items => { setProfiles(items.filter(item => item.is_active)); if (!persistedCalculation && draft.business_profile_id === 0 && items.length) setDraft(current => ({ ...current, business_profile_id: (items.find(item => item.is_default) ?? items[0]).id })); }); }, []); // eslint-disable-line react-hooks/exhaustive-deps
