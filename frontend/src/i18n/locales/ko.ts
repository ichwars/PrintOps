export default {
  orderUiNotFound: '레코드를 찾을 수 없습니다.',
  ...
  printers: {
    ...
    camWall: {
      ...
      page: {
        tokenRejected: '이 카메라 월 토큰이 거부되었거나 만료되었습니다.',
        loadFailed: '카메라 월을 불러오지 못했습니다.'
      }
    },
    ...
    drying: {
      ...
      rotateUnavailableReason: '사용할 수 없음: 이 AMS의 슬롯 하나가 아직 공급 튜브에 필라멘트가 들어 있어 전체 유닛이 회전할 수 없습니다. 먼저 해당 슬롯에서 "필라멘트 언로드"를 실행한 후 회전을 켤 수 있습니다.'
    },
    amsBackup: {
  ...
  queue: {
    ...
    batchOrders: {
      ...
      strandedNotice: '남은 {{owed}}건 중 {{runs}}건을 추가할 수 없습니다. 이 플레이트에는 설정을 복사할 실행이 없습니다. 파일에서 한 번 추가하거나 배치를 취소하세요.',
    ...
    cancelBatchConfirmMessage: '이 배치를 종료할까요? 대기 중인 실행은 취소되고 생산하지 않은 수량은 폐기됩니다.',
    batch: {
      defaultName: '배치',
      label: '{{count}}개 항목',
    ...
    toast: {
      ...
      keptForOrder: '제거되지 않음: “{{name}}” 배치에 이 플레이트의 유일한 재사용 가능 설정이 필요합니다',
      historyKeptForOrders: '열린 배치에 필요한 설정 {{kept}}개를 유지했습니다.',
  ...
  stats: {
    ...
    energySource: {
      smartPlugLive: '스마트 플러그 실시간 합계',
      smartPlugSnapshots: '스마트 플러그 스냅샷',
      printLogs: '출력 로그',
    },
  ...
  settings: {
    ...
    tabs: {
      ...
      deviceManagementDevices: '장치',
      deviceManagementPrintProcess: '인쇄 프로세스',
      projectManagementFiles: '파일 관리',
    ...
    printerProductionSubTabDescriptions: {
      devices: '기본 프린터, 카메라, FTP 재시도 동작, 가상 프린터 엔드포인트를 관리합니다.',
      printProcess: '인쇄 기본값, 보관, 큐 동작, G-code, 슬라이서, 완료 규칙을 설정합니다.',
      pipelines: '슬라이서 파이프라인, 프리셋, 자동 준비 흐름을 관리합니다.',
      failureDetection: 'AI 실패 모니터링, 프린터별 감지 동작, 상태와 기록을 설정합니다.',
    },
    projectManagementSubTabDescriptions: {
      files: '파일 처리, 보관 모드, 디스크 경고, 프로젝트 저장 규칙을 관리합니다.',
    },
    warehouseMaterialSubTabDescriptions: {
      filament: '건조 프리셋, Spoolman 추적, 필라멘트 확인, 매핑, AMS 표시 임계값을 관리합니다.',
      catalogs: '재고 및 라벨 흐름에 사용하는 스풀과 색상 카탈로그를 관리합니다.',
    ...
    documents: {
      ...
      status: { draft: 'Draft', scheduled: 'Scheduled', active: 'Active', superseded: 'Superseded', missingDraft: '초안 없음' },
    ...
    noticePreferences: '알림',
    showDeveloperLanWarning: 'Developer LAN 경고 표시',
    showDeveloperLanWarningDesc: '연결된 프린터에 Developer LAN 모드가 활성화되어 있지 않으면 앱 상단에 경고를 표시합니다.',
    showSponsorPrompts: '후원 안내 표시',
    showSponsorPromptsDesc: '후원자와 PrintOps 커뮤니티 재원에 대한 안내를 가끔 표시합니다.',
    ...
    progressInTitle: '브라우저 탭에 인쇄 진행률 표시',
    progressInTitleHint: '프린터가 실행 중이면 탭 제목과 파비콘에 활성 인쇄 진행률이 표시됩니다.',
    ...
    cameraUrlBuilderToggle: '호스트, 포트, 경로로 URL 만들기',
    cameraUrlBuilderHost: '호스트 또는 IP',
    cameraUrlBuilderPort: '포트',
    cameraUrlBuilderPath: '경로 (예: /stream)',
    cameraUrlBuilderUsername: '사용자 이름 (선택 사항)',
    cameraUrlBuilderPassword: '비밀번호 (선택 사항)',
    cameraUrlBuilderHelp: '위의 카메라 URL 필드를 채웁니다. 수동으로 입력한 URL과 동일하게 검증 및 테스트됩니다.',
  ...
  users: {
    ...
    form: {
      ...
      groups: '그룹',
      printerScope: '프린터 접근 권한',
      allPrinters: '모든 프린터',
      printerScopeCount: '프린터 {{count}}대',
      optional: '선택사항',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      device: {
        title: 'Orca Cloud에서 PrintOps 승인',
        instruction: 'Orca Cloud를 열고 이 코드를 승인하세요. 승인하면 PrintOps가 자동으로 연결됩니다.',
        codeLabel: '페어링 코드',
...
useEmbedded: '파일의 내장 설정 사용',
  ...
  inventory: {
    ...
    bulk: {
      ...
      mergeTargetPrompt: '선택한 ID 중 대상 스풀 ID ({{ids}})',
      mergeInvalidTarget: '선택한 스풀 ID 중 하나를 대상으로 선택하세요',
      mergeSuccess: '중복 스풀 {{count}}개를 병합했습니다',
      mergeFailed: '병합 실패',
      invalidHex: '16진수 6자(RRGGBB) 또는 8자(RRGGBBAA)를 입력하세요. 그렇지 않으면 필드가 적용되지 않습니다.',
    },
    refill: '리필',
    refillPrompt: '스풀 #{{id}}에 추가한 리필 무게를 g 단위로 입력',
    refillInvalidWeight: '0 g보다 큰 무게를 입력하세요',
    refillSuccess: '스풀 리필이 기록되었습니다',
    refillFailed: '스풀 리필 실패',
  ...
  haSensors: {
    ...
    blockPrintHint: '작업은 센서가 해제될 때까지 대기열에 남습니다. Home Assistant에 연결할 수 없으면 선택한 장애 전략이 적용됩니다.',
    safeState: '안전 상태: {{state}}',
    failureStrategyLabel: 'Home Assistant를 사용할 수 없을 때',
    failureAuto: '자동(중요 안전 센서는 차단)',
    failureOpen: '인쇄 허용(fail-open)',
    failureClosed: '인쇄 보류(fail-closed)',
    failureStrategyHint: '중요 안전 센서는 최신 측정값이 없으면 대기열을 보류합니다.',
    primarySafetyWarning: 'PrintOps 인터록은 보조 제어입니다. 독립적인 물리적 보호 장치를 유지하세요.',
    failure: {
      auto: '장애 시: 자동',
      fail_open: '장애 시: 인쇄 허용',
      fail_closed: '장애 시: 인쇄 보류',
    },
    override: {
      unavailable: 'fail-closed 인터록을 사용할 수 없음: {{sensors}}',
      open: '일시적으로 무시',
      reasonLabel: '일시적 무시 사유',
      confirm: '무시 확인',
      clear: '인터록 복원',
      active: '{{user}} 사용자가 무시함: {{reason}}',
      unknownUser: '알 수 없는 사용자',
      error: '인터록 무시 상태를 변경할 수 없습니다.',
    },
    toast: {
      created: '센서를 추가했습니다',
      updated: '센서를 저장했습니다',
  ...
  smartPlugs: {
    ...
    linkTarget: '연결 대상',
    noTarget: '대상 없음 (수동 제어만)',
    targetPrinter: '프린터',
    targetDryer: '필라멘트 건조기',
    selectPrinter: '프린터 선택',
    selectDryer: '필라멘트 건조기 선택',
  ...
  notifications: {
    ...
    barkSound: '소리',
    barkLevel: '방해 수준',
    barkLevelDefault: '기본값',
    barkLevelPassive: '수동',
    barkLevelActive: '활성',
    barkLevelTimeSensitive: '시간 민감',
    barkLevelCritical: '중요',
    smtpServer: 'SMTP 서버',
    smtpPort: 'SMTP 포트',
    security: '보안',
    authentication: '인증',
  ...
  bugReport: {
    ...
    manualReportTitle: 'GitHub로 보고',
    manualReportDescription: 'PrintOps는 버그 보고에 공개 GitHub 이슈 양식을 사용합니다. 설치별 릴레이 설정이 필요 없고 제출할 내용을 직접 관리할 수 있습니다.',
    openIssueForm: 'GitHub 이슈 열기',
  ...
  failureDetection: {
    ...
    mlTokenPlaceholder: '서버가 ML_API_TOKEN 없이 실행되면 비워 두세요',
    mlTokenHint: 'Obico ml_api 컨테이너의 ML_API_TOKEN 환경 변수와 일치해야 합니다. 컨테이너가 토큰 없이 실행되면 비워 두세요.',
    test: '테스트',
    testSuccess: 'ML API에 도달 가능하며 정상입니다.',
    testSuccessTokenUnknown: 'ML API에 연결할 수 있고 정상입니다. 토큰은 확인할 수 없었습니다.',
