export default {
  ...
  nav: {
    ...
    installAppSuccess: 'PrintOpsをインストールしました',
  },
  ...
  printers: {
    ...
    camWall: {
      ...
      page: {
        tokenRejected: 'このカメラウォールトークンは拒否されたか、有効期限が切れています。',
        loadFailed: 'カメラウォールを読み込めませんでした。',
      },
    },
    // Controls
    ...
    maintenance: {
      ...
      editFieldHelp: '有効にすると、このプリンターはMQTT、キューディスパッチ、通知から一時停止されます — 修理、並列のPrintOpsインストール、または一時的な停止に役立ちます。',
    },
    // Chamber light
    ...
    drying: {
      ...
      toastNotStarted: 'プリンターはコマンドを受け付けましたが、AMS は乾燥を開始しませんでした。AMS 電源アダプターが接続され、プリンターが待機中であることを確認してください。',
      screenOnly: '乾燥はこのプリンターの画面からのみ操作できます',
      rotateTray: '乾燥中にスプールを回転',
      rotateUnavailableReason: '利用不可: このAMSのスロットにまだフィラメントが供給チューブに通されており、ユニット全体の回転が止まっています。まずそのスロットで「フィラメントをアンロード」してから、回転を有効にできます。',
    },
    amsBackup: {
    ...
    expectedSlot: {
      external: '外部スプール',
      label: '{{ams}} · スロット {{slot}}',
      title: '一時停止中の印刷はここにフィラメントを待っています',
      ariaLabel: '期待される交換スロット {{n}}',
    },
    // Filaments section
    filaments: 'フィラメント',
    // Camera
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: 'スライサーがプリンターのSDカードに.gcode.3mfファイルを残さなかったため、PrintOpsはサムネイルもスライサーメタデータも取得できませんでした。これは通常、スライサー (Bambu Studio / OrcaSlicer のデバイスタブ) で「送信ファイルを外部ストレージに保存」がオフになっていることが原因です。',
  ...
  queue: {
    ...
    batchOrders: {
      ...
      strandedNotice: '未実行の {{owed}} 件のうち {{runs}} 件を追加できません。このプレートには設定をコピーできる実行が残っていません。ファイルから一度追加するか、バッチをキャンセルしてください。',
    ...
    cancelBatchConfirmMessage: 'このバッチを終了しますか？キュー内の実行はキャンセルされ、未生産の数量は破棄されます。',
    batch: {
      defaultName: 'バッチ',
      label: '{{count}}件',
    ...
    toast: {
      ...
      keptForOrder: '削除されませんでした。「{{name}}」がこのプレートの唯一の再利用可能な設定として必要としています',
      historyKeptForOrders: '未完了のバッチに必要な {{kept}} 件の設定を保持しました。',
  ...
  stats: {
    ...
    energySource: {
      smartPlugLive: 'スマートプラグのライブ合計',
      smartPlugSnapshots: 'スマートプラグのスナップショット',
      printLogs: '印刷ログ',
    },
  ...
  settings: {
    ...
    tabs: {
      general: '一般',
      usersSecurity: 'ユーザーとセキュリティ',
      printersProduction: 'プリンターと生産',
      projectsFiles: 'プロジェクトとファイル',
      warehouseMaterial: '倉庫と材料',
      ordersCalculation: '注文と計算',
      integrations: '連携',
      operations: '運用',
      smartPlugs: 'スマートプラグ',
      notifications: '通知',
    ...
    documents: {
      ...
      status: { draft: 'Draft', scheduled: 'Scheduled', active: 'Active', superseded: 'Superseded', missingDraft: '下書きなし' },
    ...
    ldap: {
      ...
      feature3: 'ログイン時にLDAPグループがPrintOpsグループにマッピングされます',
      ...
      autoProvisionHint: '初回LDAPログイン時にPrintOpsアカウントを自動作成',
      ...
      groupMappingHint: 'LDAPグループDNをPrintOpsグループにマッピング。利用可能なグループ: ',
    ...
    localLogin: {
      ...
      disableHint: '有効にすると、SSOプロバイダーのみでサインインできます。LDAPには影響しません。復旧用のパスを残すには、サーバーで PRINTOPS_LOCAL_LOGIN=true を設定してください。',
    },
    // Queue
    ...
    preheatFilamentTargetsHint: 'PrintOpsはロード済みのAMSスロット中で最も高い目標値を選びます。PLAのみの印刷は0となり、チャンバー段階は自動でスキップされます。',
    ...
    authDisabledDescription: '認証を有効にして、ユーザーアカウントの作成、権限の管理、PrintOpsインスタンスのセキュリティを確保しましょう。',
    authDisabledMessage: '認証を有効にして、ユーザーアカウントの作成、権限の管理、PrintOpsインスタンスのセキュリティを確保しましょう。',
    ...
    disableAuthenticationMessage: '認証を無効にしますか？PrintOpsインスタンスにログインなしでアクセスできるようになります。ユーザーはデータベースに残りますが、認証は無効になります。',
    disableAuthentication: '認証を無効化',
    // Additional settings
    configurePrintOps: 'PrintOpsを設定',
    ...
    noticePreferences: '通知',
    showDeveloperLanWarning: 'Developer LAN警告を表示',
    showDeveloperLanWarningDesc: '接続済みプリンターでDeveloper LANモードが有効でない場合、アプリ上部に警告を表示します。',
    showSponsorPrompts: '支援者通知を表示',
    showSponsorPromptsDesc: '支援者とPrintOpsのコミュニティ資金に関する通知を時々表示します。',
    ...
    progressInTitle: 'ブラウザータブに印刷進捗を表示',
    progressInTitleHint: 'プリンターが実行中の場合、タブタイトルとファビコンに進捗率を表示します。',
    // Archive
    autoArchivePrints: '印刷を自動アーカイブ',
    autoArchiveDescription: '印刷完了時に3MFファイルを自動保存',
    saveThumbnailsDescription: '3MFファイルからプレビュー画像を抽出して保存',
    captureFinishPhotoDescription: '印刷完了時にプリンターカメラから写真を撮影します。PrintOps は印刷中に短いタイムラプスを記録し、ベッドが下がる前の瞬間から写真を取得できるようにします。この印刷でタイムラプスを有効にしていた場合はタイムラプスファイルが保存され、それ以外の場合は写真の取得後に自動的に削除されます。',
    ...
    cameraUrlBuilderToggle: 'ホスト・ポート・パスからURLを作成',
    cameraUrlBuilderHost: 'ホストまたはIP',
    cameraUrlBuilderPort: 'ポート',
    cameraUrlBuilderPath: 'パス (例: /stream)',
    cameraUrlBuilderUsername: 'ユーザー名（任意）',
    cameraUrlBuilderPassword: 'パスワード（任意）',
    cameraUrlBuilderHelp: '上のカメラURL欄に入力されます。手動入力した場合と同じように検証・テストされます。',
    ...
    printopsSoftware: 'PrintOpsソフトウェア',
    autoCheckDescription: '起動時に自動的に新しいバージョンを確認',
    checkNow: '今すぐ確認',
    updateAvailableVersion: 'アップデート利用可能: v{{version}}',
    releaseNotes: 'リリースノート',
    updateViaDocker: 'Docker Composeでアップデート:',
    updateViaHomeAssistant: 'アップデートはHome Assistant Supervisorによって管理されます。Home Assistantの設定→アドオン→PrintOpsを開いて新しいバージョンをインストールしてください。',
    ...
    // Network tab
    externalUrl: '外部URL',
    externalUrlDescription: 'PrintOpsがアクセス可能な外部URL。通知画像や外部連携に使用されます。',
    printopsUrl: 'PrintOpsのURL',
    ...
    mqttDescription: 'Node-RED、Home Assistant、その他の自動化システムとの統合のため、外部MQTTブローカーにPrintOpsイベントをパブリッシュ。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTPシークレットとOIDCのclient_secretsは平文で保存されています。MFA_ENCRYPTION_KEYを設定するか、書き込み可能なデータディレクトリでPrintOpsを再起動して自動生成してください。',
      ...
      migrationErrorWarning: '{{count}} 件のレガシー行を起動時に再暗号化できませんでした。サーバーログを確認し、PrintOps を再起動して再試行してください。',
    },
  ...
  hmsErrors: {
    ...
    runoutExpectedSlot: '{{ranOut}} のフィラメントが切れました。印刷は現在 {{expected}} のフィラメントを待っています。',
    runoutExpectedSlotOnly: 'フィラメントが切れました。印刷は現在 {{expected}} のフィラメントを待っています。',
    runoutSlotUnknown: 'フィラメントが切れました。必要なAMSスロットをプリンターで確認してください。',
  ...
  login: {
    ...
    twoFA: {
      ...
      instructionsTotp: '認証アプリを開いて、PrintOps用の6桁のコードを入力してください。',
  ...
  // Setup page
  setup: {
    title: 'PrintOps セットアップ',
    subtitle: 'PrintOpsインスタンスの認証を設定',
    enableAuth: '認証を有効化',
  ...
  camera: {
    ...
    diagnose: {
      ...
      info: {
        ...
        uptime: 'ストリーム稼働時間',
        tlsProxy: 'TLSプロキシ',
        go2rtcStream: 'go2rtcストリーム',
        notActive: 'このプリンターで現在アクティブなストリームはありません。',
        pipelineName: {
          go2rtc: 'go2rtc（RTSPソース、MSE + MJPEGを提供）',
          go2rtc_chamber_bridge: 'go2rtc（チャンバー画像ブリッジ、MJPEGのみ）',
          chamber_binary: 'チャンバー画像プロトコル（独自バイナリ）',
          external_mjpeg: '外部カメラ（MJPEG、go2rtc経由）',
          external_rtsp: '外部カメラ（RTSP、go2rtc経由）',
          external_snapshot: '外部カメラ（HTTPスナップショット、go2rtc経由）',
          external_usb: '外部カメラ（USB/v4l2、直接）',
        },
        player: 'プレーヤー',
        playerName: {
          mse: 'MSE（H.264、再エンコードなし）',
          mjpeg: 'MJPEG（フォールバック）',
          connecting: '接続中…',
        },
      },
  ...
  users: {
    title: 'ユーザー管理',
    subtitle: 'ユーザーとPrintOpsインスタンスへのアクセスを管理',
    backToSettings: '設定に戻る',
    createUser: 'ユーザーを作成',
    noPermission: 'このページにアクセスする権限がありません。',
    admin: '管理者',
    ...
    form: {
      ...
      groups: 'グループ',
      printerScope: 'プリンターアクセス',
      allPrinters: 'すべてのプリンター',
      printerScopeCount: '{{count}} 台のプリンター',
      optional: 'オプション',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        title: 'Orca Cloudに接続',
        description: 'Orca Cloudアカウントにサインインして、スライサープロファイルをPrintOpsに同期します。',
      },
      device: {
        title: 'Orca CloudでPrintOpsを承認',
        instruction: 'Orca Cloudを開いてこのコードを承認してください。承認するとPrintOpsが自動的に接続されます。',
        codeLabel: 'ペアリングコード',
    ...
    connectedAs: '接続中:',
    signInExpired: 'Bambu Cloud のサインイン期限が切れました。クラウドプリセットを使用するには再度サインインしてください。',
    logout: 'ログアウト',
  ...
  sponsors: {
    ...
    tagline: 'PrintOpsは無料で、これからも無料です。支援してくださる方々のおかげで、VCもクラウドロックインもありません。',
    viewSupporters: 'サポーター一覧',
    toastPrints: 'PrintOpsで{{count}}件の印刷を完了しました。PrintOpsは支援者のおかげで無料で提供されています。',
    toastCost: 'PrintOpsで{{total}}分のフィラメントを記録しました。プロジェクトを支えてくれている方々をご覧ください。',
    toastArchives: '{{count}}件の印刷をPrintOpsでアーカイブしました。独立を支えてくれている方々をご覧ください。',
    toastAnniversary: 'PrintOpsとの1周年です！プロジェクトを支えてくれている方々をご覧ください。',
    toastVersionUpdate: 'v{{version}}にアップデートされました。PrintOpsは支援者のおかげで無料で提供されています。',
  },
...
useEmbedded: 'ファイル内蔵設定を使用',
    useEmbeddedHint: '上のプロファイルではなく、設計者の設定（壁、インフィル、フィラメント）でスライスします。プリンターがファイルと一致するため利用できます。',
  ...
  inventory: {
    ...
    bulk: {
      ...
      mergeTargetPrompt: '選択したIDから対象スプールIDを指定 ({{ids}})',
      mergeInvalidTarget: '選択したスプールIDのいずれかを対象にしてください',
      mergeSuccess: '{{count}} 個の重複スプールを結合しました',
      mergeFailed: '結合に失敗しました',
      invalidHex: '16進数 6 文字（RRGGBB）または 8 文字（RRGGBBAA）を入力してください。それ以外の場合この項目は適用されません。',
    },
    refill: '補充',
    refillPrompt: 'スプール #{{id}} の補充重量をグラムで入力',
    refillInvalidWeight: '0 g より大きい重量を入力してください',
    refillSuccess: 'スプール補充を記録しました',
    refillFailed: 'スプール補充に失敗しました',
    spoolmanMixedContentTitle: 'Spoolman を HTTPS で読み込めません — ブラウザが混在コンテンツをブロックしています',
    spoolmanMixedContentBody: 'PrintOps はリバースプロキシ経由で HTTPS 配信されていますが、Spoolman の URL は HTTP のままです。ブラウザはセキュリティ上の理由で混在コンテンツをブロックするため、埋め込みの Spoolman UI を表示できません。Spoolman も HTTPS でアクセスできる必要があります。',
    spoolmanMixedContentFixReverseProxy: 'Spoolman を PrintOps と同じリバースプロキシ（Traefik / Nginx / Caddy）の後ろに HTTPS で配置し、設定で Spoolman URL を新しい HTTPS アドレスに更新してください。',
  ...
  backup: {
    ...
    repoIsPublicWarning: 'リポジトリが公開（PUBLIC）です。PrintOpsのバックアップには MQTT 認証情報、Home Assistant トークン、Prometheus トークン、Bambu Cloud のメールアドレス、K-プロファイル経由のプリンタアクセスコードが含まれます。プロバイダー側でリポジトリをプライベートに変更するまで保存はブロックされます。',
    repoVisibilityUnknown: 'リポジトリの公開設定を確認できませんでした。PrintOps はプライベートと確認できないリポジトリへのバックアップを拒否します。保存はブロックされます。',
    ...
    appSettings: 'アプリ設定',
    appSettingsDescription: 'PrintOps設定（データベース全体）',
    ...
    localBackupDescription: 'データベース、アーカイブ、アップロード、すべてのファイルを含むPrintOpsデータの完全なバックアップを作成します。',
    ...
    backupRestoredRestart: 'バックアップが復元されました。PrintOpsを再起動してください。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: 'メモはPrintOpsに保存され、プリンターには保存されません',
      syncing: 'プリンターと同期中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '仮想プリンターを有効化',
      visibleInSlicer: 'スライサーの検出リストに「PrintOps」として表示',
    ...
    remoteInterface: {
      ...
      hint: 'SSDPで広告され、TLS証明書に使用されるIPアドレスを上書きします。PrintOpsに複数のネットワークインターフェースがある場合に便利です。',
    },
    mode: {
      title: 'モード',
      archive: 'アーカイブ',
    ...
    howItWorks: {
      ...
      step2: 'アーカイブ、レビュー、キューモードでは、スライサーの「送信」ボタンを使用して3MFファイルをPrintOpsにアップロードします。スライサーは「印刷成功」と表示しますが、ファイルは保存され、印刷はされません。',
    ...
    caCert: {
      title: 'スライサー証明書',
      description: '仮想プリンターは PrintOps CA が署名した TLS 証明書を使用します。この CA 証明書をスライサーの信頼ストアに一度インポートすると接続が受け入れられます — コマンドラインからコピーする必要はありません。',
      copy: 'コピー',
      copied: 'コピーしました',
      download: 'ダウンロード',
      fingerprint: 'SHA-256',
  ...
  haSensors: {
    ...
    blockPrintHint: 'ジョブはセンサーが解除されるまでキューに残ります。Home Assistant に接続できない場合は、選択した障害戦略が適用されます。',
    safeState: '安全な状態: {{state}}',
    failureStrategyLabel: 'Home Assistant が利用できない場合',
    failureAuto: '自動（重要な安全センサーは停止）',
    failureOpen: '印刷を許可（fail-open）',
    failureClosed: '印刷を保留（fail-closed）',
    failureStrategyHint: '重要な安全センサーは最新の測定値がない場合、キューを保留します。',
    primarySafetyWarning: 'PrintOps のインターロックは補助制御です。独立した物理的保護を維持してください。',
    failure: {
      auto: '障害時: 自動',
      fail_open: '障害時: 印刷を許可',
      fail_closed: '障害時: 印刷を保留',
    },
    override: {
      unavailable: 'fail-closed インターロックが利用不可: {{sensors}}',
      open: '一時的に上書き',
      reasonLabel: '一時上書きの理由',
      confirm: '上書きを確認',
      clear: 'インターロックを復元',
      active: '{{user}} による上書きが有効: {{reason}}',
      unknownUser: '不明なユーザー',
      error: 'インターロックの上書きを変更できませんでした。',
    },
    toast: {
      created: 'センサーを追加しました',
      updated: 'センサーを保存しました',
  ...
  smartPlugs: {
    ...
    linkTarget: 'リンク先',
    noTarget: 'リンク先なし（手動制御のみ）',
    targetPrinter: 'プリンター',
    targetDryer: 'フィラメントドライヤー',
    selectPrinter: 'プリンターを選択',
    selectDryer: 'フィラメントドライヤーを選択',
    ...
    restEnergyTotalPath: '総電力量JSONパス',
    restEnergyTotalMultiplier: '総電力量乗数',
  ...
  notifications: {
    ...
    pushoverExpire: '緊急期限 (秒)',
    botToken: 'ボットトークン',
    chatId: 'チャットID',
    telegramThreadId: 'フォーラムトピックID',
    telegramThreadIdInvalid: 'フォーラムトピックIDは数値である必要があります',
    barkDeviceKey: 'デバイスキー',
    barkGroup: 'グループ',
    barkSound: 'サウンド',
    barkLevel: '通知割り込みレベル',
    barkLevelDefault: '標準',
    barkLevelPassive: 'パッシブ',
    barkLevelActive: 'アクティブ',
    barkLevelTimeSensitive: '時間指定',
    barkLevelCritical: '重大',
    smtpServer: 'SMTPサーバー',
    smtpPort: 'SMTPポート',
    security: 'セキュリティ',
    authentication: '認証',
  ...
  externalLinks: {
    ...
    mainPage: 'メインページ',
    childPageOf: '{{parent}} の子ページ',
    hiddenByParent: '{{parent}} によって非表示',
    showParentFirst: '最初に {{parent}} を表示',
    hidePage: 'ページを非表示',
    showPage: 'ページを表示',
    settingsCannotBeHidden: '設定は非表示にできません',
    noLinksConfigured: '外部リンクが設定されていません',
    deleteLink: 'リンクを削除',
    deleteConfirm: '本当に "{{name}}" を削除しますか？ この操作は元に戻せません。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'PrintOpsはDockerブリッジネットワークで実行されています。プリンター検出と仮想プリンターにはホストネットワークモードが必要です — "network_mode: host" でコンテナを再作成してください。',
        skip: 'Dockerで実行されていません — 該当しません。',
      },
      subnet: {
        title: 'ネットワークサブネット',
        pass: 'プリンターとPrintOpsは同じサブネットにあります。',
        warn: 'プリンター ({{printer_ip}}) とPrintOps ({{host_ip}}) は異なるサブネットにあります。サブネット間のルーティングが設定されていない限り、互いに到達できない可能性があります。',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: 'プリンター画面（LAN設定）からアクセスコードをコピーし直し、PrintOps のプリンター設定で更新してください。',
      },
      'ftp-connection-timeout': {
        name: 'ファイル転送の接続がタイムアウトしました',
        cause: 'PrintOps がプリンターのファイル転送ポート（FTPS 990）に到達できませんでした。ポートがブロックされているか、プリンターの電源が切れているか、別のサブネットにあります。',
        fix: 'PrintOps とプリンターの間でポート 990 がブロックされていないこと、両者が同じネットワークにあることを確認してください。',
      },
      'ftp-ssl-error': {
      ...
      'database-locked': {
        ...
        fix: 'PrintOps を外部の PostgreSQL データベースに切り替えてください。ドキュメントの PostgreSQL ガイドを参照してください。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: 'サービスが稼働中',
        fail: '仮想プリンターは有効ですが、サービスが稼働していません。PrintOps のログを確認してください — 通常はバインド IP の競合または権限エラーが原因で停止します。',
      },
      bind_interface: {
      ...
      certificate: {
        title: 'TLS 証明書',
        pass: '証明書の準備ができています。PrintOps CA 証明書（上記）がスライサーの信頼ストアにインポートされていることを確認してください。',
        fail: 'この仮想プリンターの TLS 証明書がありません。PrintOps のデータディレクトリが書き込み可能か確認してください。',
      },
    },
  },
  ...
  bugReport: {
    ...
    manualReportTitle: 'GitHub で報告',
    manualReportDescription: 'PrintOps はバグ報告に公開 GitHub Issue フォームを使用します。インストールごとのリレー設定が不要になり、送信する内容を自分で管理できます。',
    openIssueForm: 'GitHub Issue を開く',
  ...
  failureDetection: {
    ...
    mlToken: 'ML APIトークン（任意）',
    mlTokenPlaceholder: 'サーバーがML_API_TOKENなしで動作する場合は空欄にします',
    mlTokenHint: 'Obico ml_apiコンテナのML_API_TOKEN環境変数と一致する必要があります。コンテナがトークンなしで動作する場合は空欄にします。',
    test: 'テスト',
    testSuccess: 'ML API に接続でき、正常です。',
    testSuccessTokenUnknown: 'ML APIに到達でき、正常です。トークンは確認できませんでした。',
    ...
    externalUrlHint: 'ML APIはURLでカメラスナップショットを取得します。MLAPIコンテナがPrintOpsに到達できるよう、一般設定で外部URLを設定してください。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: 'MakerWorld モデルの URL を貼り付けると、Bambu Handy アプリを開かなくても PrintOps から直接インポート・印刷できます。',
    ...
    disclaimer: 'MakerWorld 連携はコミュニティで文書化された API エンドポイントを使用しています。PrintOps は MakerWorld または Bambu Lab との提携・承認関係はありません。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（リンク済み）フォルダー内のファイルはスキップされます — PrintOpsは所有していないバイトを削除しません。',    previewLoading: '対象ファイル数を確認中…',
  ...
  forecast: {
    ...
    expandSku: '{{label}}を展開',
    collapseSku: '{{label}}を折りたたむ',
    procurementTitle: '調達',
    procurementLoadingOffers: '調達先を読み込んでいます …',
    procurementOffersLoadError: '調達先を読み込めませんでした。',
    procurementRetryOffers: '調達先を再読み込み',
    procurementSuppliersLoadError: '仕入先を読み込めませんでした。',
    procurementRetrySuppliers: '仕入先を再読み込み',
    procurementSave: '調達先を保存',
    procurementSaved: '調達先を保存しました',
    procurementSaveFailed: '調達先を保存できませんでした',
    sku: 'SKU',
    // Table headers
    material: '素材',
    stock: '在庫',
