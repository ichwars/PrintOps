export default {
  ...
  nav: {
    ...
    logout: '退出登录',
    installApp: '安装应用',
    installAppSuccess: 'PrintOps 已安装',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldLabel: '维护模式',
      editFieldHelp: '启用后，此打印机将从 MQTT、队列调度和通知中暂停 — 适用于维修、并行 PrintOps 安装或临时停用。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: '切片机没有在打印机的 SD 卡上留下 .gcode.3mf 文件，因此 PrintOps 无法获取缩略图或切片机元数据。这通常是因为切片机 (Bambu Studio / OrcaSlicer 的设备选项卡) 中"将发送的文件存储在外部存储中"已关闭。',
      docsLink: '查看安装步骤 4',
      dismissLabel: '关闭此通知',
    },
  ...
  queue: {
    ...
    batchOrders: {
      ...
      strandedNotice: '仍欠的 {{owed}} 次打印中有 {{runs}} 次无法加入队列：该板已没有可复制设置的记录。请从文件将该板加入队列一次，或取消批次。',
    ...
    cancelBatchConfirmMessage: '结束此批次？队列中的打印将被取消，未生产的数量将被舍弃。',
    batch: {
      defaultName: '批次',
      label: '{{count}} 项',
    ...
    toast: {
      ...
      cancelFailed: '取消项目失败',
      removed: '队列项目已移除',
      keptForOrder: '未移除：“{{name}}”仍需要此打印作为该板唯一可复用的设置',
      historyKeptForOrders: '保留了开放批次仍需要的 {{kept}} 个设置。',
      removeFailed: '移除项目失败',
      stopped: '打印已停止',
      stopFailed: '停止打印失败',
      released: '打印已释放到队列',
  ...
  stats: {
    ...
    energySource: {
      smartPlugLive: '智能插座实时总量',
      smartPlugSnapshots: '智能插座快照',
      printLogs: '打印日志',
    },
  ...
  settings: {
    ...
    tabs: {
      general: '通用',
      usersSecurity: '用户与安全',
      printersProduction: '打印机与生产',
      projectsFiles: '项目与文件',
      warehouseMaterial: '仓库与材料',
      ordersCalculation: '订单与计算',
      integrations: '集成',
      operations: '运维',
      smartPlugs: '智能插座',
      notifications: '通知',
    ...
    documents: {
      ...
      status: { draft: 'Draft', scheduled: 'Scheduled', active: 'Active', superseded: 'Superseded', missingDraft: '无草稿' },
    ...
    ldap: {
      ...
      feature3: '登录时 LDAP 组映射到 PrintOps 组',
    ...
    localLogin: {
      ...
      disableHint: '启用后，只能通过SSO提供商登录。LDAP不受影响。在服务器上设置 PRINTOPS_LOCAL_LOGIN=true 可保留恢复通道。',
    },
    // Queue
    enableRetry: '启用重试',
    // Home Assistant
    ...
    preheatFilamentTargetsHint: 'PrintOps 会在已装入的 AMS 槽位中选择最高目标；仅 PLA 的打印推导为 0 并自动跳过腔体阶段。',
    ...
    authDisabledMessage: '启用身份验证以创建用户账户、管理权限并保护您的 PrintOps 实例。',
    ...
    disableAuthenticationMessage: '确定要禁用身份验证吗？这将使您的 PrintOps 实例无需登录即可访问。所有用户将保留在数据库中但身份验证将被禁用。',
    disableAuthentication: '禁用身份验证',
    // Additional settings
    configurePrintOps: '配置 PrintOps',
    systemDefault: '系统默认',
    ...
    showDeveloperLanWarning: '显示 Developer LAN 警告',
    showDeveloperLanWarningDesc: '当已连接的打印机未启用 Developer LAN 模式时，在应用顶部显示警告。',
    showSponsorPrompts: '显示支持者提示',
    showSponsorPromptsDesc: '偶尔显示关于支持者和 PrintOps 社区资助的提示。',
    ...
    progressInTitle: '在浏览器标签页显示打印进度',
    progressInTitleHint: '打印机运行时，标签页标题和 favicon 会显示当前打印百分比。',
    autoArchivePrints: '自动归档打印',
    autoArchiveDescription: '打印完成时自动保存3MF文件',
    saveThumbnailsDescription: '从3MF文件中提取并保存预览图像',
    captureFinishPhotoDescription: '打印完成时从打印机摄像头拍照。PrintOps 会在打印期间录制一段短延时摄影，以便从热床下降前的瞬间获取照片；如果您为本次打印启用了延时摄影，文件将保留，否则会在拍照完成后自动删除。',
    ...
    cameraUrlBuilderToggle: '通过主机、端口和路径生成URL',
    cameraUrlBuilderHost: '主机或IP',
    cameraUrlBuilderPort: '端口',
    cameraUrlBuilderPath: '路径（如 /stream）',
    cameraUrlBuilderUsername: '用户名（可选）',
    cameraUrlBuilderPassword: '密码（可选）',
    cameraUrlBuilderHelp: '将填充上方的摄像头URL字段，其验证和测试方式与手动输入的URL相同。',
    ...
    printopsSoftware: 'PrintOps 软件',
    autoCheckDescription: '启动时自动检查新版本',
    checkNow: '立即检查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '发布说明',
    updateViaDocker: '通过 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。请在 Home Assistant 中打开 设置 → 加载项 → PrintOps 以安装新版本。',
    ...
    goToBackup: '前往备份',
    externalUrl: '外部URL',
    externalUrlDescription: 'PrintOps可访问的外部URL。用于通知图像和外部集成。',
    ...
    mqttDescription: '将PrintOps事件发布到外部MQTT代理，用于与Node-RED、Home Assistant和其他自动化系统集成。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP 密钥和 OIDC client_secrets 以明文存储。请设置 MFA_ENCRYPTION_KEY 或使用可写数据目录重启 PrintOps 以自动生成。',
      ...
      migrationErrorWarning: '{{count}} 行旧数据在启动时未能重新加密。请检查服务器日志并重启 PrintOps 以重试。',
    },
  ...
  hmsErrors: {
    ...
    runoutExpectedSlot: '{{ranOut}} 中的耗材已用尽。打印现在需要 {{expected}} 中的耗材。',
    runoutExpectedSlotOnly: '耗材已用尽。打印现在需要 {{expected}} 中的耗材。',
    runoutSlotUnknown: '耗材已用尽。请在打印机上查看当前需要哪个 AMS 料槽。',
  ...
  login: {
    ...
    twoFA: {
      ...
      methodEmail: '邮箱验证码',
      methodBackup: '备用恢复码',
      instructionsTotp: '请打开您的身份验证器应用，输入 PrintOps 的 6 位验证码。',
  ...
  setup: {
    title: 'PrintOps 设置',
    subtitle: '为您的 PrintOps 实例配置身份验证',
    enableAuth: '启用身份验证',
    adminAccount: '管理员账户',
  ...
  camera: {
    ...
    diagnose: {
      ...
      info: {
        ...
        uptime: '流运行时间',
        tlsProxy: 'TLS 代理',
        go2rtcStream: 'go2rtc 流',
        notActive: '此打印机当前没有活动的流。',
        pipelineName: {
          go2rtc: 'go2rtc（RTSP 源，提供 MSE + MJPEG）',
          go2rtc_chamber_bridge: 'go2rtc（舱内图像桥接，仅 MJPEG）',
          chamber_binary: '舱内图像协议（专有二进制）',
          external_mjpeg: '外部摄像头（MJPEG，通过 go2rtc）',
          external_rtsp: '外部摄像头（RTSP，通过 go2rtc）',
          external_snapshot: '外部摄像头（HTTP 快照，通过 go2rtc）',
          external_usb: '外部摄像头（USB/v4l2，直连）',
        },
        player: '播放器',
        playerName: {
          mse: 'MSE（H.264，无需重新编码）',
          mjpeg: 'MJPEG（后备方案）',
          connecting: '连接中…',
        },
      },
  ...
  users: {
    title: '用户管理',
    subtitle: '管理用户及其对 PrintOps 实例的访问',
    backToSettings: '返回设置',
    createUser: '创建用户',
    noPermission: '您没有访问此页面的权限。',
    admin: '管理员',
    ...
    form: {
      ...
      groups: '组',
      printerScope: '打印机访问权限',
      allPrinters: '所有打印机',
      printerScopeCount: '{{count}} 台打印机',
      optional: '可选',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        ...
        description: '登录您的 Orca Cloud 账户,将切片机配置同步到 PrintOps。',
      },
      device: {
        title: '在 Orca Cloud 中批准 PrintOps',
        instruction: '打开 Orca Cloud 并批准此代码。批准后 PrintOps 会自动连接。',
        codeLabel: '您的配对代码',
    ...
    connectedAs: '已连接为',
    signInExpired: '你的 Bambu Cloud 登录已过期。请重新登录以使用云端预设。',
    logout: '退出登录',
  ...
  sponsors: {
    ...
    tagline: 'PrintOps 完全免费，并将持续免费——这要感谢主动支持的用户。没有风投，也没有云端绑定。',
    viewSupporters: '查看支持者',
    toastPrints: '你已经用 PrintOps 完成了 {{count}} 次打印。PrintOps 之所以免费，离不开支持者。',
    toastCost: '你已经用 PrintOps 追踪了 {{total}} 的耗材。看看是谁让项目保持独立。',
    toastArchives: '用 PrintOps 归档了 {{count}} 次打印。看看是谁让它保持独立。',
    toastAnniversary: '与 PrintOps 相伴一年了！看看是谁让项目保持独立。',
    toastVersionUpdate: '已更新至 v{{version}}。PrintOps 之所以免费，离不开支持者。',
  },
...
useEmbedded: '使用文件内置设置',
    useEmbeddedHint: '按设计者设置的方式切片（壁数、填充、耗材），而不是使用上方配置文件。由于你的打印机与文件匹配，因此可用。',
    autoOrient: '自动摆正模型',
  ...
  inventory: {
    ...
    bulk: {
      ...
      mergeTargetPrompt: '从所选 ID 中选择目标料盘 ID ({{ids}})',
      mergeInvalidTarget: '请选择一个已选料盘 ID 作为目标',
      mergeSuccess: '已合并 {{count}} 个重复料盘',
      mergeFailed: '合并失败',
      invalidHex: '请输入 6 位十六进制字符 (RRGGBB) 或 8 位 (RRGGBBAA)，否则该字段不会应用。',
    },
    refill: '补充',
    refillPrompt: '料盘 #{{id}} 的补充重量，单位克',
    refillInvalidWeight: '请输入大于 0 g 的重量',
    refillSuccess: '已记录料盘补充',
    refillFailed: '料盘补充失败',
    spoolmanMixedContentTitle: 'Spoolman 无法通过 HTTPS 加载 — 浏览器已阻止混合内容',
    spoolmanMixedContentBody: 'PrintOps 通过您的反向代理以 HTTPS 提供服务，但您的 Spoolman 地址仍为 HTTP。出于安全考虑，浏览器会阻止混合内容，因此嵌入式 Spoolman 界面无法加载。Spoolman 也必须通过 HTTPS 访问。',
    spoolmanMixedContentFixReverseProxy: '请将 Spoolman 置于与 PrintOps 相同的反向代理（Traefik / Nginx / Caddy）之后并启用 HTTPS，然后在设置中将 Spoolman URL 更新为新的 HTTPS 地址。',
  ...
  backup: {
    ...
    repoIsPublicWarning: '仓库为公开（PUBLIC）。PrintOps 备份包含 MQTT 凭据、Home Assistant 令牌、Prometheus 令牌、您的 Bambu Cloud 邮箱以及通过 K-profile 暴露的打印机访问代码。在您于提供商设置中将仓库改为私有之前，保存将被阻止。',
    repoVisibilityUnknown: '无法确定仓库的可见性。PrintOps 拒绝向任何未确认为私有的目标进行备份；保存将被阻止。',
    ...
    appSettings: '应用设置',
    appSettingsDescription: 'PrintOps 配置（完整数据库）',
    spoolInventory: '耗材库存',
    ...
    // Local Backup
    localBackup: '本地备份',
    localBackupDescription: '创建 PrintOps 数据的完整备份，包括数据库、档案、上传和所有文件。',
    ...
    backupRestoredRestart: '备份已恢复。请重启 PrintOps。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '备注保存在 PrintOps 中，不在打印机上',
      syncing: '与打印机同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '启用虚拟打印机',
      visibleInSlicer: '在切片软件发现中显示为"PrintOps"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未激活',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自动检测（默认）...',
      hint: '覆盖通过 SSDP 广播并在 TLS 证书中使用的 IP 地址。在 PrintOps 有多个网络接口时很有用。',
    },
    mode: {
      title: '模式',
      archive: '归档',
    ...
    caCert: {
      title: '切片软件证书',
      description: '虚拟打印机使用由 PrintOps CA 签发的 TLS 证书。将此 CA 证书导入切片软件的信任库一次，切片软件即可接受连接 — 无需再通过命令行获取。',
      copy: '复制',
      copied: '已复制',
      download: '下载',
      fingerprint: 'SHA-256',
    ...
    howItWorks: {
      ...
      step2: '在归档、审核和队列模式下，使用切片软件中的"发送"按钮将 3MF 文件上传到 PrintOps。切片软件会显示"打印成功"— 文件已存储，未打印。',
  ...
  haSensors: {
    ...
    blockPrintHint: '任务会留在队列中直到传感器恢复。Home Assistant 无法连接时将采用所选故障策略。',
    safeState: '安全状态：{{state}}',
    failureStrategyLabel: 'Home Assistant 不可用时',
    failureAuto: '自动（关键安全传感器将阻止打印）',
    failureOpen: '允许打印（fail-open）',
    failureClosed: '暂停打印（fail-closed）',
    failureStrategyHint: '关键安全传感器在没有最新读数时会暂停队列。',
    primarySafetyWarning: 'PrintOps 联锁仅为辅助控制。请保留独立的物理安全保护。',
    failure: {
      auto: '故障策略：自动',
      fail_open: '故障策略：允许打印',
      fail_closed: '故障策略：暂停打印',
    },
    override: {
      unavailable: 'fail-closed 联锁不可用：{{sensors}}',
      open: '临时越过',
      reasonLabel: '临时越过原因',
      confirm: '确认越过',
      clear: '恢复联锁',
      active: '{{user}} 已启用越过：{{reason}}',
      unknownUser: '未知用户',
      error: '无法更改联锁越过状态。',
    },
    toast: {
  ...
  smartPlugs: {
    ...
    restEnergyTotalPath: '总能耗 JSON 路径',
    restEnergyTotalMultiplier: '总能耗乘数',
  ...
  notifications: {
    ...
    botToken: '机器人令牌',
    chatId: '聊天 ID',
    telegramThreadId: '论坛主题 ID',
    telegramThreadIdInvalid: '论坛主题 ID 必须是数字',
    barkDeviceKey: '设备密钥',
    barkGroup: '分组',
    barkSound: '声音',
    barkLevel: '打扰级别',
    ...
    barkLevelPassive: '被动',
    barkLevelActive: '主动',
    barkLevelTimeSensitive: '时间敏感',
    barkLevelCritical: '紧急',
    smtpServer: 'SMTP 服务器',
    smtpPort: 'SMTP 端口',
    security: '安全',
    authentication: '认证',
  ...
  externalLinks: {
    ...
    mainPage: '主页面',
    childPageOf: '{{parent}} 的子页面',
    hiddenByParent: '通过 {{parent}} 隐藏',
    showParentFirst: '先显示 {{parent}}',
    hidePage: '隐藏页面',
    showPage: '显示页面',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'PrintOps 正在以 Docker bridge 网络运行。打印机发现和虚拟打印机需要 host 网络模式 — 请使用 "network_mode: host" 重新创建容器。',
        skip: '未在 Docker 中运行 — 不适用。',
      },
      subnet: {
        title: '网络子网',
        pass: '打印机和 PrintOps 位于同一子网。',
        warn: '打印机（{{printer_ip}}）和 PrintOps（{{host_ip}}）位于不同的子网。除非配置了子网之间的路由，否则它们可能无法相互访问。',
        skip: '无法确定子网 — 已跳过。',
      },
      mqtt_auth: {
        title: '打印机凭据',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: '从打印机屏幕（局域网设置）重新复制访问码，并在 PrintOps 的打印机设置中更新它。',
      },
      'ftp-connection-timeout': {
        name: '文件传输连接超时',
        cause: 'PrintOps 无法连接打印机的文件传输端口（FTPS 990）。该端口被阻止，或打印机已关机或位于其他子网。',
        fix: '请确保 PrintOps 与打印机之间的 990 端口未被阻止，并且两者位于同一网络。',
      },
      'ftp-ssl-error': {
        name: '安全文件传输握手失败',
      ...
      'database-locked': {
        ...
        fix: '请将 PrintOps 切换到外部 PostgreSQL 数据库。参见文档中的 PostgreSQL 指南。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: '服务正在运行',
        fail: '虚拟打印机已启用，但其服务未运行。请检查 PrintOps 日志 — 通常是绑定 IP 冲突或权限错误使其停止。',
      },
      bind_interface: {
        title: '绑定网络接口',
      ...
      certificate: {
        title: 'TLS 证书',
        pass: '证书已就绪。请确保已将 PrintOps CA 证书（上方）导入切片软件的信任库。',
        fail: '此虚拟打印机的 TLS 证书缺失。请检查 PrintOps 数据目录是否可写。',
      },
    },
  },
  ...
  bugReport: {
    ...
    submit: '提交',
    startLogging: '开始调试日志',
    manualReportTitle: '通过 GitHub 报告',
    manualReportDescription: 'PrintOps 使用公开的 GitHub issue 表单处理错误报告。这样无需为每个安装配置 relay，并且提交内容由你控制。',
    openIssueForm: '打开 GitHub issue',
  ...
  failureDetection: {
    ...
    mlTokenPlaceholder: '如果服务器未使用 ML_API_TOKEN，请留空',
    mlTokenHint: '必须与 Obico ml_api 容器的 ML_API_TOKEN 环境变量一致。如果容器未使用令牌，请留空。',
    test: '测试',
    testSuccess: 'ML API 可访问且正常。',
    testSuccessTokenUnknown: 'ML API 可访问且状态正常。无法检查令牌。',
    ...
    externalUrlHint: 'ML API 通过 URL 获取摄像头快照。请在通用设置中设置外部 URL，以便 ML API 容器可以访问 PrintOps。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: '粘贴 MakerWorld 模型链接，即可直接在 PrintOps 中导入并打印 —— 无需切换到 Bambu Handy 应用。',
    ...
    disclaimer: 'MakerWorld 集成使用由社区记录的 API 接口。PrintOps 与 MakerWorld 或 Bambu Lab 没有从属或认可关系。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（链接）文件夹中的文件将被跳过 — PrintOps 从不删除不属于自己的字节。',    previewLoading: '正在检查匹配的文件数量…',
