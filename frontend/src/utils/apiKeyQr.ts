 ...
 *
 * The QR encodes the PrintOps base URL and the freshly-created API key together
 * so a mobile client can scan one code to configure both.
 *
 * Payload contract (fixed — bump `v` if it changes):
 *   printops://config?v=1&url=<encodeURIComponent(baseUrl)>&key=<encodeURIComponent(apiKey)>
 */
 ...
 *
 * @param baseUrl Origin a client uses to reach PrintOps (origin only, no path).
 * @param apiKey  Raw API key string.
 */
export function buildApiKeyQrPayload(baseUrl: string, apiKey: string): string {
  return (
    `printops://config?v=${API_KEY_QR_VERSION}` +
