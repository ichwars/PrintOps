...
OrcaSlicer and/or Bambu Studio CLI. PrintOps's **Slice** action calls
these to slice models server-side, no desktop slicer required.

This folder is **optional**. PrintOps works without it — Slice falls back
...
the sidecar on a separate x86_64 box and point PrintOps at it via the
**Sidecar URL** field — the sidecar doesn't need to live next to PrintOps.

## Ports

| Service | Default host port | Why this port |
|---|---|---|
| `orca-slicer-api` | **3003** | PrintOps's virtual-printer feature reserves 3000 and 3002 |
...
In the PrintOps UI: **Settings → Slicer**:
...
`BAMBU_STUDIO_API_URL` environment defaults from PrintOps's config.

## Where the images live

Pre-built images are published to two registries on every PrintOps
stable release:
...
Each release also publishes a versioned tag (`:printops-X.Y.Z`) so you
can pin to the sidecar that shipped alongside a specific PrintOps
release — set `SIDECAR_TAG=printops-0.2.5` in `.env`.

Both images are built from the
[`maziggy/orca-slicer-api`](https://github.com/maziggy/orca-slicer-api)
fork (`printops/profile-resolver` branch). The fork patches AFKFelix's
...
To roll back to the sidecar that shipped with a previous PrintOps
release, set `SIDECAR_TAG=printops-X.Y.Z` in `.env` and re-run the two
commands above.

## Troubleshooting

- **`address already in use` on port 3000 or 3002** — PrintOps's
