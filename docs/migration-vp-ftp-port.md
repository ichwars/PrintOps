...
a firewall, ensure these ports are open between the slicer and PrintOps.
...
3. **Verify systemd service** has `AmbientCapabilities=CAP_NET_BIND_SERVICE`:
   ```bash
   systemctl cat printops | grep AmbientCapabilities
   ```
   If missing, add it to the `[Service]` section.

4. **Restart PrintOps.** Verify FTP binds to port 990:
   ```bash
   grep "FTPS on" logs/printops.log
