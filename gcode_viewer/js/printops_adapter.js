/**
 * printops_adapter.js
 * Bridges OctoPrint-PrettyGCode to PrintOps's API.
 *
 ...
 *   - PrintOps WebSocket → fromCurrentData bridge
 *   - File picker backed by PrintOps's library API
 *   - Settings load/save via plugin settings endpoint
 *
 * What works:
 *   - Full 3D GCode visualisation
 *   - Dark mode and all dat.GUI settings
 *   - File selection from PrintOps's file library
...
(function () {
    ...
    // -------------------------------------------------------------------------
    // 4. fetch() interceptor — rewrite OctoPrint paths to PrintOps
    // -------------------------------------------------------------------------
    ...
    window.fetch = function (resource, init) {
        ...
        if (url) {
            ...
            // OctoPrint file download  →  PrintOps library download
            newPath = newPath.replace(
                /^\/?downloads\/files\/local\/__printops_file_(\d+)$/,
                API_BASE + '/library/files/$1/download'
            );
            // OctoPrint file download  →  PrintOps archive gcode (specific plate)
            newPath = newPath.replace(
                /^\/?downloads\/files\/local\/__printops_archive_(\d+)_plate(\d+)$/,
                API_BASE + '/archives/$1/gcode?plate=$2'
            );
            // OctoPrint file download  →  PrintOps archive gcode (first plate)
            newPath = newPath.replace(
                /^\/?downloads\/files\/local\/__printops_archive_(\d+)$/,
                API_BASE + '/archives/$1/gcode'
            );
            // OctoPrint file download  →  PrintOps library file gcode
            // (sliced LibraryFile — extracts embedded gcode from .gcode.3mf
            // or returns plain .gcode). Plate is ignored upstream for now.
            newPath = newPath.replace(
                /^\/?downloads\/files\/local\/__printops_libgcode_(\d+)(?:_plate\d+)?$/,
            ...
            // Inject auth header for all PrintOps API calls
    ...
    function loadArchiveById(archiveId, plate) {
        // Pretygcode downloads /downloads/files/local/__printops_archive_<id>(_plate<N>)
        ...
        if (viewModel && viewModel.fromCurrentData) {
            viewModel.fromCurrentData({
                job: {
                    file: {
                        path: '__printops_archive_' + archiveId + plateSuffix,
    ...
    function loadLibraryFileById(fileId, plate) {
        ...
        if (viewModel && viewModel.fromCurrentData) {
            viewModel.fromCurrentData({
                job: {
                    file: {
                        path: '__printops_libgcode_' + fileId + plateSuffix,
                        date: currentFileDate,
                    },
                    estimatedPrintTime: null,
                },
    ...
    function init() {
        ...
        console.log('[PrettyGCode] PrintOps adapter initialised');
