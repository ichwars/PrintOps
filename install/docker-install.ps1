<#
.SYNOPSIS
    PrintOps Docker installation script for Windows (Docker Desktop).
...
.PARAMETER InstallPath
    Installation directory. Default: $env:USERPROFILE\printops
...
function Write-Banner {
    ...
    Write-Host '   PrintOps - Docker Installation (Windows)' -ForegroundColor Cyan
    Write-Host '==========================================================' -ForegroundColor Cyan
    Write-Host ''
}
...
function Get-ComposeFile {
    ...
    if ($Build) {
        ...
        Invoke-WebRequest `
            -Uri 'https://raw.githubusercontent.com/ichwars/PrintOps/main/docker-compose.yml' `
...
function Start-PrintOps {
    Info 'Starting PrintOps container...'
    if ($Build) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "    $DockerCompose logs printops" -ForegroundColor Yellow
        exit 1
    }
    ...
    while ($attempts -lt 15) {
        ...
        if ($ps -match 'Up') { Ok 'PrintOps container is running'; return }
        if ($ps -match 'Exited') {
...
# Defaults
if (-not $InstallPath) { $InstallPath = Read-Default 'Installation directory' (Join-Path $env:USERPROFILE 'printops') }
...
Write-Host '==========================================================' -ForegroundColor Green
Write-Host ''
Write-Host "  Access PrintOps:  http://localhost:$Port" -ForegroundColor Cyan
...
Write-Host "    Logs:     cd `"$InstallPath`"; $DockerCompose logs -f printops"
...
Write-Host ''
Write-Host '  Documentation:    https://github.com/ichwars/PrintOps/wiki' -ForegroundColor Cyan
Write-Host ''
