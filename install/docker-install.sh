#!/usr/bin/env bash
#
# PrintOps Docker Installation Script
# Supports: Linux (all distros), macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/ichwars/PrintOps/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
#   Unattended:   ./docker-install.sh --path /opt/printops --port 8000 --yes
#
# Options:
...
show_help() {
    echo "PrintOps Docker Installation Script"
    echo ""
    ...
    echo "    ./docker-install.sh --path /srv/printops --port 3000 --tz America/New_York --yes"
    echo ""
...
create_install_dir() {
    ...
    if ! mkdir -p "$INSTALL_PATH" 2>/dev/null; then
        # The default `/opt/printops` (and any other root-owned parent) needs
...
download_compose_file() {
    ...
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
        ...
        if [[ -d ".git" ]]; then
            ...
            git clone https://github.com/ichwars/PrintOps.git .
        fi
    else
        # Just download the compose file
        curl -fsSL -o docker-compose.yml \
            https://raw.githubusercontent.com/ichwars/PrintOps/main/docker-compose.yml
    fi
...
create_env_file() {
    ...
    cat > .env << EOF
# PrintOps Docker Configuration
# Generated by docker-install.sh on $(date)

# Port PrintOps runs on
PORT=$PORT
...
start_container() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "PrintOps container is running"
            return 0
        fi
        ...
        if $DOCKER_CMD ps -a | grep -q "Exited"; then
            ...
            log_info "Check logs with: $DOCKER_CMD logs printops"
            return 1
        fi
...
main() {
    ...
    echo -e "    Logs:    cd $INSTALL_PATH && $DOCKER_CMD logs -f printops"
    ...
    echo ""
    echo -e "  ${BOLD}Data location:${NC}  Docker volumes (printops_data, printops_logs)"
    echo ""
