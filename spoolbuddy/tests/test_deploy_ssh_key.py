"""Tests for daemon.main._deploy_ssh_key — PrintOps key sync.

Background: PrintOps generates an ed25519 keypair under its data dir and ships
the public half to the SpoolBuddy daemon over the registration/heartbeat
response. The daemon writes that key into ~/.ssh/authorized_keys so PrintOps
can SSH in to drive remote updates. Whenever PrintOps's keypair rotates (data
...
must drop the old entries and pick up the new one — otherwise:
  ...
  2. Stale PrintOps-tagged keys pile up over time, eroding the security
     boundary (any prior keypair PrintOps held is permanently authorized).
...
CURRENT_KEY = "ssh-ed25519 AAAACURRENT printops-spoolbuddy"
STALE_KEY_1 = "ssh-ed25519 AAAASTALE1 printops-spoolbuddy"
STALE_KEY_2 = "ssh-ed25519 AAAASTALE2 printops-spoolbuddy"
...
class TestDeploySshKey:
    ...
    def test_replaces_all_prior_printops_tagged_keys(self, tmp_path):
    ...
    def test_preserves_unrelated_user_keys(self, tmp_path):
        """Only PrintOps-tagged keys get replaced — user's own keys stay."""
        ssh_dir = tmp_path / ".ssh"
        ssh_dir.mkdir()
        auth_keys = ssh_dir / "authorized_keys"
